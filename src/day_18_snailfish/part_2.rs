...
fn join_two_snailfish_numbers(lhs: &[(u8, usize)], rhs: &[(u8, usize)]) -> Vec<(u8, usize)> {
    let mut joined_snailfish_number: Vec<(u8, usize)> = Vec::new();
    joined_snailfish_number.extend(lhs);
    joined_snailfish_number.extend(rhs);
    joined_snailfish_number
        .iter_mut()
        .for_each(|(depth, _)| *depth += 1);
    joined_snailfish_number
}
...
fn find_highest_possible_magnitude(snailfish_numbers: &[Vec<(u8, usize)>]) -> usize {
    let mut highest_magnitude: usize = 0;

    for snailfish_number in snailfish_numbers {
        for other_snailfish_number in snailfish_numbers {
            if snailfish_number == other_snailfish_number {
                continue;
            }

            let mut new_sfn = join_two_snailfish_numbers(snailfish_number, other_snailfish_number);

            while explode_snailfish_number(&mut new_sfn) || split_snailfish_number(&mut new_sfn) {}

            calculate_magnitude(&mut new_sfn);
            highest_magnitude = highest_magnitude.max(new_sfn[0].1);
        }
    }
...
pub fn solve() -> String {
    let snailfish_numbers = process_data("./input/18.txt");
    let result = find_highest_possible_magnitude(&snailfish_numbers);
    format!("Day 18: Snailfish (Part 2) answer: {}", result)
}

#[cfg(test)]
mod tests {
    use crate::day_18_snailfish::part_2::*;

    #[test]
    fn solve_with_test_data() {
        let snailfish_numbers = process_data("./test_input/18.txt");
    ...
    fn join_snailfish_numbers_together() {
        let sfn_1: Vec<(u8, usize)> = Vec::from([(1, 1), (1, 2)]);
        let sfn_2: Vec<(u8, usize)> = Vec::from([(2, 3), (2, 4), (1, 5)]);
        let joined_sfn: Vec<(u8, usize)> = Vec::from([(2, 1), (2, 2), (3, 3), (3, 4), (2, 5)]);

        let result = join_two_snailfish_numbers(&sfn_1, &sfn_2);
        assert_eq!(result, joined_sfn);
    }
