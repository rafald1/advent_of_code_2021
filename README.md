...
Part 1 of this puzzle was straightforward, but part 2 introduced a complex problem. The game involved a 3-sided quantum die that, when rolled, split the universe into multiple copies - one for each possible outcome of the die. Each turn, the die was rolled 3 times, producing all 27 possible combinations. Fortunately, only the sum of these 3 rolls mattered, which reduced the number of possibilities to 7 (the sums of 3 and 9 occur only once each, the sums of 4 and 8 occur 3 times each, the sums of 5 and 7 occur 6 times each, and the sum of 6 occurs 7 times). It took some time to get the recursive function right, and the initial implementation was rather slow. Implementing memoization significantly reduced the time.

### [Day 22](https://adventofcode.com/2021/day/22)
I had a lot of difficulties choosing the correct approach to this problem, which involved operating on ranges in 3D space. As a warm-up, I implemented a naive solution for part 1, which handled each cube of a cuboid separately and used a `HashSet` to store cubes that were on.

Part 2 required understanding what happens when cuboids intersect and how to properly represent this information. For illustration, if we have two cuboids with dimensions 3x3x3 cubes (a volume of 27 cubes each) and they are both in the "on" state, when they intersect minimally with only 1 cube, instead of splitting one of the cuboids into pieces to reflect the 1 missing cube, I decided to keep both cuboids in their original form and add a new cuboid of the size of the intersection with the "off" state. In this case, I have a cuboid of size 1x1x1 with the "off" state. I noticed that there are no cases where you are instructed to turn on or turn off a cuboid that is completely contained by another cuboid of the same state. This observation allowed me to simplify the code and make it more readable. When counting the volume of all cuboids, I add those with the "on" state and subtract those with the "off" state.

### [Day 23](https://adventofcode.com/2021/day/23)
It has been 3 days. I spent a lot of time trying to choose the proper data structure to handle all necessary operations, such as calculating the distance between burrow locations, moving amphipods around, checking if the path is unobstructed, and verifying if the final configuration has been reached. This was a great puzzle to solve by hand but hard to code, because regardless of the approach, the code was difficult to read. Even the changes introduced in Part 2 required some hardcoded adjustments to the input data.

I opted out of using the A* search algorithm because the necessary heuristics to guide the search made the code even less readable. In my final solution, I used a single array to store all the burrow states, where the first 11 elements represented hallway states, and the remaining elements represented room 0, 1, 2, and 3 states. I also used two additional structs to improve readability. In representing states in the burrow: 0 is empty, and 1, 2, 3, and 4 are different types of amphipods.

The process of finding valid paths was as follows:
